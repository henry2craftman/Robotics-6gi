...
{
    object dataLock = new object(); // 뮤텍스"Mutex"(잠금장치,키) 어떤 공유자원
    public string latestPlcData = "";
    ...
    void Start()
    {
        ...
        Task.Run(UpdatePlcDataAsync); // 2. 서브스레드에서 값을 바꿈.
    }
    ...
    IEnumerator CoUpdate()
    {
        while (true)
        {
            ...
            // latestPlcData : 백그라운드 스레드와 메인스레드가 공유하는 값
            lock (dataLock)
            {
    ...
    // Task로 실행한 async/await에는 Unity관련 함수가 실행되지 않음.
